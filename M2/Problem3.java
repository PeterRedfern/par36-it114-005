...
public class Problem3 {
    ...
    static <T> void bePositive(T[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            {
                if (arr instanceof Integer[]) { //par36 - 9/22/23 (checks if it is an integer array)
                    output[i] = Math.abs((Integer) arr[i]); // uses math.abs to output positive numbers in an int array
                } else if (arr instanceof Double[]) {  // checks if it is a double array
                    output[i] = Math.abs((Double) arr[i]); // uses math.abs to output positive number in a double array
                } else if (arr instanceof String[]) { // checks if it is a string array
                    output[i] = String.valueOf(Math.abs(Integer.parseInt((String) arr[i]))); // uses String.valueOf with math.abs
                }                                                                            // to parse the ints and output a positive number in a string array
            }
        }
        // end edit section
